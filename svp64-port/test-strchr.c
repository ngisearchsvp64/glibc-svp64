...
test_main (void)
{
  ...
  // Use MAX_SIZE 256 instead of default 2048 for speed
  ...
  for (i = 1; i < 8; ++i)
    {
      do_test (0, 16 << i, MAX_SIZE, 0, MIDDLE_CHAR);
      do_test (i, 16 << i, MAX_SIZE, 0, MIDDLE_CHAR);
    }

  for (i = 1; i < 8; ++i)
    {
      do_test (i, 64, MAX_SIZE, 0, MIDDLE_CHAR);
      do_test (i, 64, MAX_SIZE, 0, BIG_CHAR);
    }
